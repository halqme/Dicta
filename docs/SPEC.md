...
- Only small streaming-capable models are offered as preview models.
...
Settings expose only model/language combinations actually supported by the installed FluidAudio version.

Preview choices are further restricted to small streaming-capable models suitable for always-resident use. The concrete model catalog is supplied by a FluidAudio adapter rather than duplicated throughout UI code.

The UI uses a concrete compatibility catalog rather than free-form model IDs. With FluidAudio 0.15.7, the small resident preview catalog is English-only (Parakeet EOU 120M). Japanese therefore has no realtime preview model in the MVP and the HUD displays only the listening state.
...
Implemented in the initial Xcode project:
...
4. Final-model installation, recording-start warm-up, long-form Cohere transcription, serial final inference, and actor-owned 10-minute eviction.
